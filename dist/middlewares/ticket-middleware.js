...
function validateCreateRequest(req, res, next) {
    ...
    if (!bookingId || isNaN(Number(bookingId)) || Number(bookingId) <= 0) {
        return next(new app_error_1.AppError(['Booking ID is required and must be a positive number'], http_status_codes_1.StatusCodes.BAD_REQUEST));
    }
    // Validate flightId (required)
    if (!flightId || isNaN(Number(flightId)) || Number(flightId) <= 0) {
        return next(new app_error_1.AppError(['Flight ID is required and must be a positive number'], http_status_codes_1.StatusCodes.BAD_REQUEST));
    }
    // Validate passengerId (required)
    if (!passengerId || isNaN(Number(passengerId)) || Number(passengerId) <= 0) {
        return next(new app_error_1.AppError(['Passenger ID is required and must be a positive number'], http_status_codes_1.StatusCodes.BAD_REQUEST));
    }
    // Validate ticketNumber (required)
    if (!ticketNumber || typeof ticketNumber !== 'string' || ticketNumber.trim() === '') {
        return next(new app_error_1.AppError(['Ticket number is required and must be a non-empty string'], http_status_codes_1.StatusCodes.BAD_REQUEST));
    }
    // Enforce ticketNumber max length per model (20)
    if (typeof ticketNumber === 'string' && ticketNumber.trim().length > 20) {
        return next(new app_error_1.AppError(['Ticket number must not exceed 20 characters'], http_status_codes_1.StatusCodes.BAD_REQUEST));
    }
    // Validate class (required)
    if (!ticketClass || typeof ticketClass !== 'string' || ticketClass.trim() === '') {
        return next(new app_error_1.AppError(['Ticket class is required and must be a non-empty string'], http_status_codes_1.StatusCodes.BAD_REQUEST));
    }
    ...
    if (req.body.seatId !== undefined && req.body.seatId !== null) {
        if (isNaN(Number(req.body.seatId)) || Number(req.body.seatId) <= 0) {
            return next(new app_error_1.AppError(['Seat ID must be a positive number'], http_status_codes_1.StatusCodes.BAD_REQUEST));
        }
    }
    next();
}
function validateUpdateRequest(req, res, next) {
    ...
    if ('bookingId' in body) {
        if (isNaN(Number(body.bookingId)) || Number(body.bookingId) <= 0) {
            return next(new app_error_1.AppError(['Booking ID must be a positive number'], http_status_codes_1.StatusCodes.BAD_REQUEST));
        }
    }
    // Validate flightId if provided
    if ('flightId' in body) {
        if (isNaN(Number(body.flightId)) || Number(body.flightId) <= 0) {
            return next(new app_error_1.AppError(['Flight ID must be a positive number'], http_status_codes_1.StatusCodes.BAD_REQUEST));
        }
    }
    // Validate passengerId if provided
    if ('passengerId' in body) {
        if (isNaN(Number(body.passengerId)) || Number(body.passengerId) <= 0) {
            return next(new app_error_1.AppError(['Passenger ID must be a positive number'], http_status_codes_1.StatusCodes.BAD_REQUEST));
        }
    }
    ...
    if ('ticketNumber' in body) {
        if (typeof body.ticketNumber !== 'string' || body.ticketNumber.trim() === '') {
            return next(new app_error_1.AppError(['Ticket number must be a non-empty string'], http_status_codes_1.StatusCodes.BAD_REQUEST));
        }
    ...
    if ('class' in body) {
        if (typeof body.class !== 'string' || body.class.trim() === '') {
            return next(new app_error_1.AppError(['Ticket class must be a non-empty string'], http_status_codes_1.StatusCodes.BAD_REQUEST));
        }
    }
