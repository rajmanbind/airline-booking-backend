...
export function validateCreateRequest(req: Request, res: Response, next: NextFunction) {
  ...
  if (!seatNumber || typeof seatNumber !== 'string' || seatNumber.trim() === '') {
    return next(new AppError(['Seat number is required and must be a non-empty string'], StatusCodes.BAD_REQUEST));
  }
  // Enforce seatNumber max length per model (10)
  if (typeof seatNumber === 'string' && seatNumber.trim().length > 10) {
    return next(new AppError(['Seat number must not exceed 10 characters'], StatusCodes.BAD_REQUEST));
  }

  // Validate class (required)
  if (!seatClass || typeof seatClass !== 'string' || seatClass.trim() === '') {
    return next(new AppError(['Seat class is required and must be a non-empty string'], StatusCodes.BAD_REQUEST));
  }

  // Validate isWindowSeat (required)
  if (typeof isWindowSeat !== 'boolean') {
    return next(new AppError(['isWindowSeat is required and must be a boolean'], StatusCodes.BAD_REQUEST));
  }

  // Validate isAisleSeat (required)
  if (typeof isAisleSeat !== 'boolean') {
    return next(new AppError(['isAisleSeat is required and must be a boolean'], StatusCodes.BAD_REQUEST));
  }
...
export function validateUpdateRequest(req: Request, res: Response, next: NextFunction) {
  ...
  if ('seatNumber' in body) {
    if (typeof body.seatNumber !== 'string' || body.seatNumber.trim() === '') {
      return next(new AppError(['Seat number must be a non-empty string'], StatusCodes.BAD_REQUEST));
    }
  ...
  if ('class' in body) {
    if (typeof body.class !== 'string' || body.class.trim() === '') {
      return next(new AppError(['Seat class must be a non-empty string'], StatusCodes.BAD_REQUEST));
    }
  }
  ...
  if ('isWindowSeat' in body) {
    if (typeof body.isWindowSeat !== 'boolean') {
      return next(new AppError(['isWindowSeat must be a boolean'], StatusCodes.BAD_REQUEST));
    }
  }
  ...
  if ('isAisleSeat' in body) {
    if (typeof body.isAisleSeat !== 'boolean') {
      return next(new AppError(['isAisleSeat must be a boolean'], StatusCodes.BAD_REQUEST));
    }
  }
